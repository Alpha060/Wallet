...
try {
    ...
    // -------------------------------------------------------------
        ...
        try {
            $insStmt = $pdo->prepare('
                INSERT INTO users (email, password_hash, name, referred_by, is_admin, is_active)
                VALUES (:email, :password_hash, :name, :referred_by, FALSE, TRUE)
    ...
    // -------------------------------------------------------------
        ...
        if ($route === 'auth/me' && $method === 'GET') {
            ...
            $stmt = $pdo->prepare('
                ...
                VALUES (:userId, :amount, :proofUrl, :txId, \'pending\')
                RETURNING id, status
            ');
            $stmt->execute([
            ...
            foreach ($expired as $inv) {
                ...
                try {
                    // Credit user balance
                    $balStmt = $pdo->prepare('UPDATE users SET wallet_balance = wallet_balance + :price WHERE id = :userId RETURNING wallet_balance');
                    $balStmt->execute(['price' => $inv['purchase_price'], 'userId' => $inv['user_id']]);
                    $balanceAfter = (int)$balStmt->fetchColumn();
            ...
            try {
                ...
                $limitStmt = $pdo->prepare('
                    ...
                    WHERE user_id = :userId AND product_id = :productId AND purchased_at::date = CURRENT_DATE
                ');
                ...
                $insStmt = $pdo->prepare('
                    INSERT INTO user_investments (user_id, product_id, purchase_price, expires_at)
                    VALUES (:userId, :productId, :price, :expiresAt)
                    RETURNING id
                ');
                $insStmt->execute([
                    'userId' => $currentUser['id'],
                ...
                recordWalletLedger($currentUser['id'], $product['price'], 'debit', 'investment_purchase', 'user_investments', $inv['id'], $newBalance, 'Yield asset purchase');
            ...
            try {
                ...
                $limitStmt = $pdo->prepare('
                    SELECT COUNT(id) FROM user_investments 
                    WHERE user_id = :userId AND product_id = :productId AND is_sold = TRUE AND sold_at::date = CURRENT_DATE
                ');
            ...
            try {
                ...
                $chkStmt = $pdo->prepare('SELECT id FROM ad_watch_log WHERE user_investment_id = :id AND watched_at::date = CURRENT_DATE');
                ...
                // Log ad watch
                $insStmt = $pdo->prepare('INSERT INTO ad_watch_log (user_investment_id, user_id, reward_amount) VALUES (:invId, :userId, :reward) RETURNING id');
                $insStmt->execute(['invId' => $investmentId, 'userId' => $currentUser['id'], 'reward' => $reward]);
                $adWatchId = $insStmt->fetchColumn();

                // Credit balance
                $balStmt = $pdo->prepare('UPDATE users SET wallet_balance = wallet_balance + :reward WHERE id = :userId RETURNING wallet_balance');
            ...
            try {
                ...
                // Deduct balance
                $balStmt = $pdo->prepare('UPDATE users SET wallet_balance = wallet_balance - :amount WHERE id = :id RETURNING wallet_balance');
                $balStmt->execute(['amount' => $amount, 'id' => $currentUser['id']]);
                $balanceAfter = (int)$balStmt->fetchColumn();

                // Create request
                $insStmt = $pdo->prepare('
                    INSERT INTO withdrawal_requests (user_id, amount, bank_details, status, payment_method, verified_details)
                    VALUES (:userId, :amount, :bank, \'pending\', :method, TRUE)
                    RETURNING id
                ');
                $insStmt->execute([
                    'userId' => $currentUser['id'],
                ...
                $withdrawalId = $insStmt->fetchColumn();
        ...
        // -------------------------------------------------------------
            ...
            if ($route === 'admin/statistics' && $method === 'GET') {
                ...
                $params = [];
                if (!empty($search)) {
                    $where .= ' AND (email ILIKE :search OR name ILIKE :search OR mobile_number ILIKE :search)';
                ...
                if ($action === 'approve' && $method === 'POST') {
                    ...
                    try {
                        ...
                        // Inline wallet ledger insert (NOT using helper - its try/catch can corrupt PG transactions)
                        $pdo->prepare('
                        ...
                        // Inline admin audit insert (same reason - avoid helper's try/catch inside transaction)
                        $pdo->prepare('
