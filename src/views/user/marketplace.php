...
$title = __("AeroPay - Marketplace");
$description = __("Invest in high-yield assets to secure daily rewards");
...
<div class="modal-overlay" id="purchase-modal">
    <div class="modal-content">
        <h3 style="font-weight: 800; margin-bottom: 16px;"><?= __('Confirm Purchase') ?></h3>
        <p style="font-size: 0.95rem; color: var(--muted); margin-bottom: 24px; line-height: 1.5;" id="purchase-modal-text"></p>
        <div style="display: flex; gap: 12px;">
            <button class="btn-secondary" onclick="closePurchaseModal()" style="flex: 1;"><?= __('Cancel') ?></button>
            <button class="btn-primary" id="purchase-confirm-btn" style="flex: 1;"><?= __('Confirm Purchase') ?></button>
        </div>
    </div>
</div>
...
<script>
    ...
    async function fetchMarketplace() {
        try {
            ...
            data.products.forEach(p => {
                ...
                card.innerHTML = `
                    <div class="roi-badge"><span class="badge-roi">${p.dailyRewardPercent}<?= __('% Daily ROI') ?></span></div>
                    ...
                    <p style="color: var(--muted); font-size: 0.8rem; margin-bottom: 16px;"><?= __('Duration:') ?> ${p.durationDays} <?= __('Days | Watch Time:') ?> ${p.adWatchSeconds}<?= __('s') ?></p>
                    ...
                    <div style="display: flex; align-items: center; justify-content: space-between; margin-top: auto; border-top: 1px solid var(--border); padding-top: 16px;">
                        <div>
                            <span style="font-size: 0.75rem; color: var(--muted); display: block;"><?= __('Purchase Price') ?></span>
                            <span style="font-size: 1.2rem; font-weight: 800; color: var(--foreground);">${formatRupees(p.price)}</span>
                        </div>
                        <button class="btn-primary" onclick="openPurchaseModal('${p.id}', '${p.name}', ${p.price})" style="padding: 10px 20px;"><?= __('Buy') ?></button>
                    </div>
                `;
    ...
    function openPurchaseModal(id, name, price) {
        ...
        document.getElementById('purchase-modal-text').innerHTML = `<?= __('Are you sure you want to purchase the asset "') ?><strong>${name}</strong><?= __('" for ') ?><strong>${formatRupees(price)}</strong><?= __('?<br>Confirming will automatically debit this amount from your available balance.') ?>`;
    ...
    document.getElementById('purchase-confirm-btn').addEventListener('click', async () => {
        ...
        try {
            ...
            Toast.show('<?= __('Asset purchased successfully!') ?>');
            closePurchaseModal();
