...
try {
    ...
    $depStmt = $pdo->prepare("INSERT INTO deposit_requests (id,user_id,amount,payment_proof_url,transaction_id,status) VALUES (:id,:u,10000,'/uploads/diag.png','DIAGTX','pending')");
    $depStmt->execute(['id' => $depositId, 'u' => $user['id']]);
    $deposit = ['id' => $depositId, 'status' => 'pending'];
    ...
    // wallet_ledger
    $pdo->prepare('INSERT INTO wallet_ledger (id,user_id,amount,direction,entry_type,reference_table,reference_id,balance_after,note,created_by) VALUES (:id,:u,:a,:d,:e,:rt,:ri,:ba,:n,:c)')
        ->execute(['id'=>generateUUID(),'u'=>$u['id'],'a'=>(int)$dep['amount'],'d'=>'credit','e'=>'deposit_approved','rt'=>'deposit_requests','ri'=>$depositId,'ba'=>$newBalance,'n'=>'Deposit approved by admin','c'=>$admin['id']]);
    echo "wallet_ledger OK\n";

    $pdo->prepare('INSERT INTO admin_audit_logs (id,admin_id,action,target_table,target_id,details) VALUES (:id,:a,:ac,:t,:ti,:d)')
        ->execute(['id'=>generateUUID(),'a'=>$admin['id'],'ac'=>'deposit_approve','t'=>'deposit_requests','ti'=>$depositId,'d'=>json_encode(['amount'=>$dep['amount'],'userId'=>$dep['user_id']])]);
